import { ProductosTO } from './service/productosto.interface';
...
export class ProductosComponent implements OnInit {
  ...
  listarProductos() {
    ...
    this._productosService.listarProductos().subscribe((data) => {
      ...
      $('#dttableProductos').DataTable().clear();
      $('#dttableProductos').DataTable().rows.add(data).draw();
      $('#dttableProductos').DataTable().columns.adjust().draw();
    });
  ...
  verProducto(id: string, tipo: number) {
    $('.overlayCargue').show();
    this._productosService.datosProducto(id).subscribe((data) => {
      this.producto = data;
      this.categoria._id = data.categoria[0]._id;
  ...
  crearProducto() {
    $('.overlayCargue').show();
    this._productosService.crearProducto(this.producto).subscribe((data) => {
      this.producto = new ProductosTO();
      this.listarProductos();
      this.modalService.dismissAll();
    });
  ...
  cambiarEstado(id: string, estado: number) {
    ...
    this.producto.estado = estado == 1 ? 2 : 1;
    Swal.fire({
      title: 'Estas seguro?',
      text: 'Cambiaras el estado del Producto!',
      icon: 'info',
    ...
    $('#dttableProductos').DataTable({
      ...
      createdRow: (row: Node, data: any | Object, index: number) => {
        $(row)
          ...
          .off('click')
          .on('click', () => {
            this.verProducto('' + data._id + '', 2);
          });
        $(row)
          .find('.editar')
          .off('click')
          .on('click', () => {
            this.verProducto('' + data._id + '', 3);
          });
        $(row)
          .find('.cambiar')
          .off('click')
